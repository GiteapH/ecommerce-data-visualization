...
<template>
    <div>
        <echartDecoration :question-content="questionContent" :title="title">
            <div id="timeLine" :style="{ width:  `${width}`, height: `${height}` }"></div>
        </echartDecoration>
    </div>
</template>
...
import { setTimeConfig } from '@/utils/js/echartsConfig/purchaseSituation/timeLine'
...
const chartDraw = (address,time)=>{
    ...
    setTimeConfig(address,time).then(reOption => {
        title.value =  `${address==null?'国内':address}用户时间段购买总量`
        myChart.hideLoading()
