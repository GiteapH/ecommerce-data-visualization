...
<template>
    <div>
        <echartDecoration :question-content="questionContent" :title="title">
            <div id="timePriceBubble" :style="{ width: `${width}`, height: `${height}` }"></div>
        </echartDecoration>
    </div>
</template>
...
import { setgetPriceConfig } from '@/utils/js/echartsConfig/purchaseSituation/timePricebubble'
...
const title = ref('国内价格段-时间段的购买总量')

const chartDraw = (address,time)=>{
    var chartDom = document.getElementById('timePriceBubble');
    ...
    setgetPriceConfig(address,time).then(reOption => {
        title.value = `${address==null?'国内':address}价格段-时间段的购买总量`
        myChart.hideLoading()
